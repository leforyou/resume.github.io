...
<!--修改地址-->
  <div class="container">
    <div class="box margin-top bg-fff">
      <ul class="main-padding-lr fz28">
        ...
        <li class="border-btm">
          ...
          <p class="key-value" :class="{'fc-999':area==null}">{{area}}</p>
    ...
    <div class="wrap-btns main-padding-lr">
      <p class="btn btn-sure flex-center fz30 fc-fff main-bg" @click="saveAddress()">确认保存</p>
      <p class="btn btn-del flex-center fz30 fc-666" @click="deleteAddress()">删除地址</p>
    </div>
...
export default {
  ...
  onLoad: function(options) {
    ...
    if(options.addressInfo){
      ...
      this.addressInfo.NvrFd1 = '广东省,广州市,天河区';
      this.region = this.addressInfo.NvrFd1.split(',');
      this.area = this.region.join("  ");
    }
  },
  methods: {
    bindRegionChange(e){
    ...
    async deleteAddress(){
      //删除地址
      let flag = await this.$modal('确定要删除地址吗');
      if(flag == false)return;
      var urlData = {
        'ActionName':'DeleteAddressByID',
        'DID':this.addressInfo.ID
      };
