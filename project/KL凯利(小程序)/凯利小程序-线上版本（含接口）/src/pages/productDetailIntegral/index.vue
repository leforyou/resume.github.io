  ...
  <div class="container" v-if="prouctDetail">
    ...
    <div class="info main-padding-lr">
      <div class="box">
        ...
        <div class="price">
          <span class="new main-color fz36">{{prouctDetail.JiFen}}分</span>
        </div>
    ...
    <productDetailSizeIntegral ref="size" :prouctDetail="prouctDetail" :PID='PID' :totalIntegral='totalIntegral'><!--商品规格--></productDetailSizeIntegral>
    ...
    <div class="foot">
      ...
      <div class="list buy-now sub-bg" :class="{'bg-888':buyState == false}" @click="shopping('buyNow')">
        <span class="fz30 fc-fff hide" :class="{'show':buyState == true}">立即兑换</span>
        <span class="fz30 fc-fff hide" :class="{'show':buyState == false}">不可兑换</span>
      </div>
      <!--<div class="list buy-now">
        <span class="fz30 sub-color" @click="shopping('buyNow')">立即购买</span>
      </div>
      <div class="list sub-bg add-cart">
        <span class="fz30 fc-fff" @click="shopping('addCart')">加入购物车</span>
      </div>-->
    </div>
...
export default {
  ...
  onShareAppMessage: function (res) {
    ...
    return {
      ...
      path: `pages/productDetailIntegral/main?${share.msg.shareIdName}=${store.state.login.memberid}&PID=${this.PID}`,
  ...
  components: {
    swiperProductDetail,productDetailEvaluate,productDetailSuggest,productDetailSizeIntegral,productDetailParameter,wxParses
  },
  ...
  methods: {
    ...
    buyStateFn(){
        if(this.totalIntegral>this.prouctDetail.JiFen){
          this.buyState = true;
        }else{
          this.buyState = false;
        }
