 ...
 *2.js设置滑动区域x:y大于3才能滑动，且不能上下滑动，上下滑动时，左右不滑动
 *3.当是一张图片，就不允许左右滑动 
 *4.允许设置是数字或点
...
function slider(obj){
	...
	obj.time = Math.max(obj.time,1200);
	...
	for(var i = 0; i < aDOM.length; i++) {
		...
		imgDom[i].setAttribute('onload','fitImgSlider(this)');
		fitImgSlider(imgDom[i]);
	}
	//导航点
	...
	setDotClass(0);
	//定时器
	var num = 1,timeToLeft = 500, setIval;
	...
	//var startX = 0,startY=0,endX,endY,startTime,endTime,dVal = 0;
	var startX=0 ,endX=0, startY=0, endY=0, startTime=0, endTime=0, ratioXY=0, dVal = 0;
	...
	function move(event){
		...
		//var dy = (e.screenY || e.targetTouches[0].screenY) - startY;
		//console.log(dy + "  " + dVal)
	...
	function end(e) {
		...
		if(30<endTime && endTime < 280 && ratioXY > 3){
			timeD();
		}else if(endTime >= 280){
			endUp();
		}
		startX=0 ,endX=0, startY=0, endY=0, startTime=0, endTime=0, ratioXY=0, dVal = 0;
